...
(() => {
  ...
  const current = document.querySelector('#hx-current');
  const hits = [...hero.querySelectorAll('.hx-project-hit')];
  let highlightTimer;
  function select(n) {
    selected=(n+4)%4;current.textContent=String(selected+1).padStart(2,'0');
  ...
  hero.querySelector('.hx-index').addEventListener('click',()=>select(selected+1));
  ...
  function resize(){
    const dpr=Math.min(devicePixelRatio,1.6),w=Math.round(stage.clientWidth*dpr),h=Math.round(stage.clientHeight*dpr);
    if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;gl.viewport(0,0,w,h);}
  }
  function render(now){
    frame=0;if(!loaded||!visible||document.hidden||motion.matches)return;
    const elapsed=last?Math.min(now-last,50):16;last=now;clock+=elapsed*.001;
