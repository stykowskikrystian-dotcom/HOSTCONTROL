...
(() => {
  ...
  function fitCaptions() {
    ...
    const scale=stage.clientWidth/reference[0];
    const w=220*scale,h=240*scale;
    hero.querySelectorAll('.hx-panel').forEach((panel,index)=>{
      const to=quads[index].map(([x,y])=>[x*stage.clientWidth/reference[0],y*stage.clientHeight/reference[1]]);
